#' @title Calculate and write maximum runs
#' @description Function for calculate and write maximum runs according to a fire isochrone map (poluygons) and a wind direction per hour table
#'
#'
#' @param AllPols spatVector object with polygons of fire progression
#' @param CRSproject crs of the spatVector
#' @param nameID name of polygon unique ID field
#' @param nameFeho name of hour ID field
#' @param WindTablexHour table with wind direction per hour. default is NULL if no wind direction is provided
#' @param flagRuns parameter to indicate whether both pol and wind runs are calculated (values='both', 'pols' or 'wind'). Default is 'both'
#' @param CreateSharedFrontLines parameter to indicate whether front lines file needs to be created. Default is T
#'
#' @return it writes one or two shapefiles with runs
#' @import terra
#' @import reshape2
#' @export
#'
#'
Runs <- function(AllPols, CRSproject=terra::crs(AllPols), nameID, nameFeho, WindTablexHour=NULL, flagRuns='both', CreateSharedFrontLines='T') {
  ...
  for (i in 1:max(terra::na.omit(AllPolsWithHours$codi_hora))) {
    ...
    if (flagRuns %in% c("both", "wind")) {
      if (!is.null(nrow(PerPoligonMaxims_wind))) { # continue only if there is a Run, if not stop
        ...
        ppmax <- PerPoligonMaxims_wind[which(PerPoligonMaxims_wind$Distance == max(PerPoligonMaxims_wind$Distance)), ][1, ]
        # Createline
        Inner <- ppmax[, c("CoordXFrom", "CoordYFrom")]
        names(Inner) <- c("x", "y")
        Outer <- ppmax[, c("CoordXTo", "CoordYTo")]
        names(Outer) <- c("x", "y")
        xy <- as.matrix(rbind(Inner, Outer))
        LinXY <- terra::vect(xy, "lines", crs = CRSproject, atts = ppmax[, c(1, 2, 3, 6, 11, 12, 13)])
        Nameslist[[i]] <- LinXY
    ...
    if (flagRuns %in% c("both", "pols")) {
      if (!is.null(nrow(PerPoligonMaxims_pol))) { # continue only if there is a Run, if not stop
        # Select Maximum Run in all polygons. The first if there is more than one.
        ppmax_pol <- PerPoligonMaxims_pol[which(PerPoligonMaxims_pol$Distance == max(PerPoligonMaxims_pol$Distance)), ][1, ]
        # Createline
        Inner <- ppmax_pol[, c("CoordXFrom", "CoordYFrom")]
        names(Inner) <- c("x", "y")
        Outer <- ppmax_pol[, c("CoordXTo", "CoordYTo")]
        names(Outer) <- c("x", "y")
        xy <- as.matrix(rbind(Inner, Outer))
        LinXY_pol <- terra::vect(xy, "lines", crs = CRSproject, atts = ppmax_pol[, c(1, 2, 3, 6, 11, 12, 13)])
        Nameslist_pol[[i]] <- LinXY_pol
      } # end if there is a run
    } # end runspol
  } # end loop per hour
